...
async def test_get_metric(authenticated_client: AsyncClient, test_metrics, db_session):
    ...
    # Perform API request
    response = await authenticated_client.get(f"{settings.API_V1_STR}/metrics/{metric.id}")
...
@pytest.mark.asyncio(loop_scope="session")
async def test_get_metrics_by_coin(authenticated_client: AsyncClient, test_coin, test_metrics, db_session):
    ...
    db_result = await db_session.execute(select(Metric).where(Metric.coin_id == test_coin.id))
    ...
    # Perform API request
    response = await authenticated_client.get(f"{settings.API_V1_STR}/metrics/coin/{test_coin.id}")
    ...
    data = response.json()
    assert len(data) == len(test_metrics), f"❌ Mismatch: Expected {len(test_metrics)}, got {len(data)}"

    for i, metric in enumerate(test_metrics):
        assert data[i]["id"] == str(metric.id), f"❌ Mismatch in metric ID at index {i}"
        assert data[i]["coin_id"] == str(metric.coin_id), f"❌ Mismatch in coin ID at index {i}"
        assert data[i]["market_cap"]["value"] == metric.market_cap["value"], f"❌ Market Cap mismatch at index {i}"
        assert data[i]["market_cap"]["currency"] == "USD", f"❌ Currency mismatch at index {i}"
